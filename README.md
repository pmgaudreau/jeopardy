...
| `config.js` | Firebase credentials only (gitignored; generated by `build.sh` on Netlify) |
| `config.example.js` | Template — copy to `config.js` and fill in your Firebase credentials |
| `helpers.js` | Shared avatar definitions and helper functions used by all pages |
| `tokens.css` | Design tokens (colors, radii, fonts) and shared CSS primitives (buttons, inputs, pills) used by all pages |
...
This app is intentionally **open by design** — no user authentication, no per-user write rules. That's fine for a single-evening trivia game with a known crowd, but you should know what it means before deploying for a larger audience:

- **Anyone with the URL can connect.** The Firebase Web SDK config ships in every page. With the rules above, any visitor can read game state, modify any team's score, delete any team mid-game, or spawn unlimited rooms.
- **The "Super Admin" passphrase is cosmetic.** It's hard-coded in `superadmin.html` and visible in page source. It exists to gate the admin UI from casual eyes, not to authenticate. Treat it as a "are you sure?" prompt, not a security boundary.
- **Use short-lived room codes.** Pick a fresh code per event, and use the superadmin page to delete the room when you're done so it can't be poked at later.

If you ever need real protection (a paid event, public tournament, etc.), the right next step is to add Firebase Authentication and rewrite the database rules to require auth for writes. That would be a larger change than the current static-HTML setup supports.
