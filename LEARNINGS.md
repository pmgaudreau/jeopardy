...
**Mute toggle from admin.** Admin's three top bars (board / question / reveal views) each have an always-visible `🔊 Sound` button. Clicking it flips `state.soundMuted` via `safeWrite`. Board's `onStateChange()` mirrors the flag onto the SFX module via `SFX.setMuted()`. When `muted=true`, `tone()`, `squishyJoin()`, and `thinkMusic.start()` all bail out, and any currently-playing music is stopped. This lets the host silence the room from admin without walking to the projector — but it does **not** unlock the autoplay gate (the projector still needs that first click).
